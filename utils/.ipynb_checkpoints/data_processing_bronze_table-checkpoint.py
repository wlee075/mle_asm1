...
def process_bronze_datalake(directory_path, dates_list, processing_function, spark, csv_file_path):
...
def process_bronze_table(snapshot_date_str, bronze_lms_directory, spark, csv_file_path):
    ...
    try:
        if "financial" in bronze_lms_directory:
            ...
            df = df.withColumn("snapshot_date", to_date(col("snapshot_date"), "M/d/yy")).filter(F.col('snapshot_date') == snapshot_date.strftime("%Y-%m-%d")) 
        else:
            df = spark.read.csv(csv_file_path, header=True, inferSchema=True) \
        ...
        return
    df_with_metadata = df \
        .withColumn("_metadata_file_name", F.input_file_name()) 
